...
void OpenCV_hough_transform(unsigned char* srcBuf, int iw, int ih, unsigned char* outBuf, int nw, int nh)
{
    ...
    cv::Vec3b lower_yellow, upper_yellow;
    // lower_yellow = Vec3b(20, 80, 100);//threshold1, threshold1);
    // upper_yellow = Vec3b(30, 255, 255);

    lower_yellow = Vec3b(10, 0, 50);//threshold1, threshold1);  //adjust last value
    upper_yellow = Vec3b(50, 255, 255);


    cv::Mat srcHLS;
    cv::cvtColor(srcRGB,srcHLS,COLOR_BGR2HLS);

    cv::Mat img_mask;
    cv::inRange(srcHLS, lower_yellow, upper_yellow, img_mask); 



    cv::Mat img_result;
    cv::bitwise_and(srcRGB, srcRGB, img_result, img_mask); //(src1, src2, dst, mask)
        //->return RGB
    

    cv::Mat srcGRAY;
    cv::cvtColor(img_result,srcGRAY,COLOR_BGR2GRAY);
    ...
    //cv::HoughLines(contours, lines, 1, PI/180, // 단계별 크기 (1과 π/180에서 단계별로 가능한 모든 각도로 반지름의 선을 찾음)
        //80);  // 투표(vote) 최대 개수
	cv::HoughLinesP(contours, lines, 1, PI/180,80,50,10);
    
    // 선 그리기
    cv::Mat result(contours.rows, contours.cols, CV_8UC3, lineColor);   //white color
    ...
    while (it!=lines.end()) 
    {
        ...
        if ((theta < PI/180*70. || theta > PI/180*110)) // 수직 행
        {
            /*cv::Point pt1(rho/cos(theta), 0); // 첫 행에서 해당 선의 교차점   
            cv::Point pt2((rho-result.rows*sin(theta))/cos(theta), result.rows);
            // 마지막 행에서 해당 선의 교차점
            cv::line(img_result, pt1, pt2, lineColor, 1); // 하얀 선으로 그리기*/

			cv::Point pt1((*it)[0],(*it)[1]);
   			cv::Point pt2((*it)[2],(*it)[3]);
   			cv::line(img_result, pt1, pt2, 1);
   			++it;
			}
    ...
    cv::resize(img_result, dstRGB, cv::Size(nw, nh), 0, 0, CV_INTER_LINEAR);
}
